...
describe('append', () => {
  ...
  it('adds multiple lines in order', () => {
    ...
    expect(next.output.map((l) => l.text)).toEqual([
      'Line 1',
      'Line 2',
      'Line 3',
    ]);
  ...
  it('appends to existing output rather than replacing it', () => {
    const state: GameState = {
      ...baseState,
      output: [{ type: 'output' as const, text: 'Existing' }],
    };
...
describe('locationLines', () => {
  ...
  // Bug 4 regression: graceful fallback for unknown locationId
  it('returns a safe fallback when location id is unknown', () => {
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    const lines = locationLines(baseState, 'nonexistent-place');
    expect(lines.length).toBeGreaterThan(0);
    expect(lines.some((l) => /unmapped/i.test(l))).toBe(true);
    expect(errorSpy).toHaveBeenCalledWith(
      expect.stringContaining('nonexistent-place')
    );
