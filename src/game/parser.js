/**
 * Parses a raw text command string into a structured command object.
 *
 * Returns: { verb, noun?, source?, target?, raw? }
 *
 * verbs:
 *   NORTH SOUTH EAST WEST UP DOWN
 *   LOOK INVENTORY HELP QUIT ASSEMBLE
 *   EXAMINE TAKE DROP WEAR
 *   USE  (with source + target)
 *   TALK_TO SAY
 *   UNKNOWN
 */
...
export function parseCommand(raw) {
  ...
  if (SHORTHANDS[upper]) return { verb: SHORTHANDS[upper] }

  // Full directions
  if (DIRECTIONS.has(upper)) return { verb: upper }
