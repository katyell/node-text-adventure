...
describe('handleTake', () => {
  it('returns prompt when no noun given', () => {
    expect(handleTake(makeState(), undefined).output.at(-1)!.text).toMatch(
      /take what/i
    );
  });
...
describe('handleDrop', () => {
  it('returns prompt when no noun given', () => {
    expect(handleDrop(makeState(), undefined).output.at(-1)!.text).toMatch(
      /drop what/i
    );
  });
...
describe('handleWear', () => {
  it('returns prompt when no noun given', () => {
    expect(handleWear(makeState(), undefined).output.at(-1)!.text).toMatch(
      /wear what/i
    );
  });
  ...
  it('returns error when item not in inventory', () => {
    const next = handleWear(makeState({ inventory: [], wearing: [] }), 'jeans');
    expect(next.output.at(-1)!.text).toMatch(/don't have/i);
  });

  // Bug 2 regression
  it('returns "already wearing" when item is already worn', () => {
    const state = makeState({ wearing: ['tshirt'] });
    const next = handleWear(state, 'tshirt');
    expect(next.output.at(-1)!.text).toMatch(/already wearing/i);
    // No duplicate added
    expect(next.wearing.filter((i) => i === 'tshirt')).toHaveLength(1);
  });
...
describe('handleUse', () => {
  it('returns format hint when source missing', () => {
    expect(
      handleUse(makeState(), undefined, 'book').output.at(-1)!.text
    ).toMatch(/USE <item> WITH/i);
  });

  it('returns format hint when target missing', () => {
    expect(
      handleUse(makeState(), 'book', undefined).output.at(-1)!.text
    ).toMatch(/USE <item> WITH/i);
  });

  it('returns error when source not held', () => {
    const next = handleUse(
      makeState({ wearing: [], inventory: [] }),
      'tshirt',
      'coffee cup'
    );
  ...
  // Bug 1 regression: one-shot uses with setFlag + repeatText

  it('sets the flag on first successful use (setFlag form)', () => {
    // tshirt + coffee cup is defined as { setFlag: 'coffeeCupClean', ... }
    const state = makeState({
      wearing: ['tshirt'],
      locationItems: { cafe: ['coffee cup'] },
      flags: {},
    ...
    const next = handleUse(state, 'tshirt', 'coffee cup');
    expect(next.flags.coffeeCupClean).toBe(true);
    expect(next.output.at(-1)!.text).toMatch(/clean the coffee cup/i);
  });

  it('shows repeatText on subsequent uses once the flag is set', () => {
    const state = makeState({
      wearing: ['tshirt'],
      locationItems: { cafe: ['coffee cup'] },
      flags: { coffeeCupClean: true },
    });
    const next = handleUse(state, 'tshirt', 'coffee cup');
    expect(next.output.at(-1)!.text).toMatch(/already clean/i);
    // Flag stays true; no duplicate state change
    expect(next.flags.coffeeCupClean).toBe(true);
  });
});
